...
class StatisticsDisplay:
  """This class keeps track of the min/average/max measurements
     and displays them"""

  def __init__(self, weatherData):
    self.temperatures = [] # list of all recorded temperatures
    self.humidities = [] # list of all recorded humidity measurements
    self.pressures = [] # list of all recorded pressure measurements

    self.weatherData = weatherData # Saves the reference inside attribute
  ...
  def get_stats(self, units):
    ...
    message = (
      ...
      + f"Min {measurement}: {sum(values / len(values))} {units}"
    )
  ...
  def display(self):
    if len(self.temperatures) > 0:
      ...
      temp_message = "No temperature data"
    print(temp_message)

    if len(self.humidities) > 0:
      humidity_message = self.get_stats("[%]")
    else:
      humidity_message = "No humidity data"
    print(humidity_message)

    if len(self.poressures) > 0:
      pressure_message = self.get_stats("")
    else:
      pressure_message = "No pressure data"
    print(pressure_message)

class ForecastDisplay(Observer):
  """This class shows the weather forecast based on
  current temperature, humidity, and pressure data"""

  def __init(self, weatherData):

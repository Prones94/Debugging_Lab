...
def good_cholestrol(total_cholostrol, ldl, triglyceride):
    return total_cholostrol < 200 and ldl < 100 and triglyceride < 150

def high_cholestrol(total_cholostrol, ldl, triglyceride):
    return not (total_cholostrol > 240 or ldl > 160 or triglyceride >= 200)

def borderline_cholestrol(total_cholostrol, ldl, triglyceride):
    return not (200 < total_cholostrol < 240 or 130 < ldl < 160 or 150 <= triglyceride < 200)

if good_cholestrol(total_cholostrol, ldl, triglyceride):
    # good level
    print('*** Good level of cholestrol ***')
elif high_cholestrol(total_cholostrol, ldl, triglyceride):
    # High cholestrol level
    print('*** High cholestrol level ***')
    print('start taking pills such as statins')
    print('start TLC diet')
elif borderline_cholestrol(total_cholostrol, ldl, triglyceride):
    #TLC_diet
